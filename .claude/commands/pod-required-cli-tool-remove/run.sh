...
removed_tools = [tool for tool in existing_tools if tool in requested_set]
missing_tools = [tool for tool in requested_tools if tool not in existing_set]
updated_tools = [tool for tool in existing_tools if tool not in requested_set]

if section_index is not None:
    lines[section_index:section_end] = build_section_lines(updated_tools)
    workspace_path.write_text("".join(lines), encoding="utf-8")

print(f"Workspace updated: {workspace_path}")
print("removed: " + (", ".join(removed_tools) if removed_tools else "(none)"))
print("not_present: " + (", ".join(missing_tools) if missing_tools else "(none)"))
PY
